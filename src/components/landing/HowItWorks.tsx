...
const STEPS = [
  { n: "01", key: "upload", icon: "upload", tone: "var(--v-blue)" },
  { n: "02", key: "style", icon: "grid", tone: "var(--v-violet)" },
  { n: "03", key: "generate", icon: "sparkle-fill", tone: "var(--acc)" },
] as const;
...
export default async function HowItWorks() {
  ...
  return (
    <Section id="how" style={{ background: "var(--bg-1)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
      <SectionHead center tag={t.howItWorks.tag} title={t.howItWorks.title} sub={t.howItWorks.sub} />
      <div className="ab-grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, marginTop: 52, position: "relative" }}>
        ...
        {STEPS.map((s, i) => (
          <div key={s.n} style={{ textAlign: "center", position: "relative" }}>
            ...
            <div className="ab-h3" style={{ fontSize: 21, marginTop: 24 }}>{t.howItWorks.steps[s.key].title}</div>
            <div className="ab-body" style={{ fontSize: 14.5, marginTop: 10, maxWidth: 280, marginLeft: "auto", marginRight: "auto" }}>{t.howItWorks.steps[s.key].desc}</div>
          </div>
        ))}
      </div>
    </Section>

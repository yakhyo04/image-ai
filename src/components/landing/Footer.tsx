...
export default async function Footer() {
  ...
  const COLUMNS = [t.footer.columns.product, t.footer.columns.company, t.footer.columns.resources];
  return (
    <Section id="footer" pad="32px 100px 56px">
      ...
      <div className="ab-footer-grid" style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 1fr", gap: 40, paddingBottom: 40 }}>
        <div>
          ...
          <div className="ab-body" style={{ fontSize: 13.5, marginTop: 16, maxWidth: 260 }}>{t.footer.tagline}</div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap", paddingTop: 28, borderTop: "1px solid var(--border)" }}>
        <span className="ab-body" style={{ fontSize: 13 }}>{t.footer.copyright}</span>
        <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
          <a href="#" className="ab-nav-link" style={{ fontSize: 13 }}>{t.footer.privacy}</a>
          <a href="#" className="ab-nav-link" style={{ fontSize: 13 }}>{t.footer.terms}</a>
      ...
      <div className="ab-news" style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 28, padding: 20, background: "var(--bg-1)", border: "1px solid var(--border)", borderRadius: 16, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 220 }}>
          <div className="ab-h4" style={{ fontSize: 16 }}>{t.footer.newsletterTitle}</div>
          <div className="ab-body" style={{ fontSize: 13, marginTop: 3 }}>{t.footer.newsletterSub}</div>
        </div>
        <div style={{ display: "flex", gap: 8, width: 380, maxWidth: "100%" }}>
          ...
          <button className="ab-btn ab-btn-primary">{t.footer.subscribe}</button>
        </div>
      </div>
    </Section>
  );

...
import type { Tool } from "./toolsData";

function specFont(k: string) {
  return k.includes("cost") || k.includes("resolution") || k.includes("time") || k.includes("Batch") || k.includes("Colorways")
    ? "var(--font-mono)"
    : "var(--font)";
}

export default function FeatureDetail({ tool: d }: { tool: Tool }) {
  return (
    <div style={{ background: "var(--bg)", color: "var(--t-1)", minHeight: "100dvh" }}>
      <Nav />
      ...
      <Section id={`${d.id}-hero`} pad="40px 100px 64px" style={{ overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 30, position: "relative" }}>
          <Link href="/#features" className="ab-nav-link" style={{ fontSize: 13 }}>Features</Link>
          <Icon name="chevron-right" size={14} style={{ color: "var(--t-4)" }} />
          <span style={{ fontSize: 13, color: "var(--t-1)", fontWeight: 500 }}>{d.name}</span>
        </div>
        <div className="ab-detail-hero" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center", position: "relative" }}>
          <div>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 30, flexWrap: "wrap" }}>
              <Link href="/dashboard" className="ab-btn ab-btn-grad ab-btn-lg">Try {d.name.split(" ")[0]} free <Icon name="arrow-right" size={17} stroke={2.2} /></Link>
              <a href="#" className="ab-btn ab-btn-ghost ab-btn-lg"><Icon name="play" size={13} /> Watch demo</a>
            </div>
            ...
            {d.heroType === "beforeafter" ? (
              <div className="ab-card" style={{ padding: 14, borderRadius: 24, boxShadow: "var(--sh-pop)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 6px 12px" }}>
                  <div style={{ display: "flex", gap: 6 }}>{["var(--err)", "var(--warn)", "var(--ok)"].map((c, i) => <div key={i} style={{ width: 9, height: 9, borderRadius: "50%", background: c, opacity: 0.7 }} />)}</div>
              ...
              <div className="ab-detail-showcase" style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gridTemplateRows: "1fr 1fr", gap: 14, height: 440 }}>
                {d.gallery.map((g, i) => (
      ...
      <Section id={`${d.id}-how`} pad="72px 100px" style={{ background: "var(--bg-1)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
        ...
        <div className="ab-grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24, marginTop: 48, position: "relative" }}>
          <div className="ab-how-line" style={{ position: "absolute", top: 38, left: "17%", right: "17%", height: 2, background: "repeating-linear-gradient(90deg, var(--border-mid) 0 8px, transparent 8px 16px)" }} />
          {d.steps.map((s, i) => (
      ...
      <Section id={`${d.id}-caps`} pad="80px 100px">
        ...
        <div className="ab-grid-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16, marginTop: 44 }}>
          {d.caps.map((c, i) => (
      ...
      <Section id={`${d.id}-gallery`} pad="0 100px 80px">
        <div className="ab-gallery-head" style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 36 }}>
          <SectionHead tag="Examples" title={`${d.name} in action`} />
          <Link href="/#gallery" className="ab-btn ab-btn-ghost">View full gallery <Icon name="arrow-up-right" size={16} /></Link>
        </div>
        <div className="ab-grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 18, alignItems: "start" }}>
          {d.gallery.map((g, i) => (
      ...
      <Section id={`${d.id}-use`} pad="80px 100px" style={{ background: "var(--bg-1)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)" }}>
        <div className="ab-detail-use" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 56 }}>
          <div>
            ...
            <div className="ab-card" style={{ marginTop: 32, overflow: "hidden", background: "var(--bg)" }}>
              {d.specs.map(([k, v], i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", borderBottom: i < d.specs.length - 1 ? "1px solid var(--border)" : "none" }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 600, color: "var(--t-1)", fontFamily: specFont(k) }}>{v}</span>
                </div>
              ))}
            </div>
      ...
      <Section id={`${d.id}-faq`} pad="80px 100px">
        <div className="ab-faq-grid" style={{ display: "grid", gridTemplateColumns: "0.8fr 1.2fr", gap: 64 }}>
          <div>
            ...
            <a href="#" className="ab-btn ab-btn-ghost" style={{ marginTop: 16 }}><Icon name="send" size={15} /> Ask support</a>
          </div>
      ...
      <Section id={`${d.id}-cta`} pad="0 100px 64px">
        <div style={{ position: "relative", borderRadius: 32, overflow: "hidden", background: "var(--grad)", padding: "60px 56px", textAlign: "center", color: "var(--acc-ink)" }}>
          ...
          <div style={{ position: "relative" }}>
            <div className="ab-h1 ab-cta-title" style={{ fontSize: 42, color: "var(--acc-ink)" }}>Ready to try {d.name}?</div>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 30, flexWrap: "wrap" }}>
              <Link href="/dashboard" className="ab-btn ab-btn-lg" style={{ background: "var(--acc-ink)", color: "var(--acc)" }}>Start generating free <Icon name="arrow-right" size={17} stroke={2.2} /></Link>
              <Link href="/#pricing" className="ab-btn ab-btn-lg" style={{ background: "oklch(0 0 0 / 0.12)", color: "var(--acc-ink)", border: "1px solid oklch(0 0 0 / 0.2)" }}>See pricing</Link>
            </div>
          </div>
        </div>
      </Section>

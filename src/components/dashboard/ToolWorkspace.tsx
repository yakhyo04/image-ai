...
export default function ToolWorkspace({ toolKey }: { toolKey: string }) {
  ...
  async function generate() {
    if (!image) {
      setError(tl.uploadFirst.replace("{label}", cfg.sourceLabel.toLowerCase()));
      return;
    }
      ...
      <div className="ab-dash-cols" style={{ display: "grid", gridTemplateColumns: "316px 1fr 296px", minHeight: "100%" }}>
        ...
        <div style={{ borderRight: "1px solid var(--border)", padding: 20 }}>
          ...
          <button onClick={generate} disabled={busy} className="ab-btn ab-btn-primary ab-btn-full ab-btn-lg" style={{ marginTop: 20, opacity: busy ? 0.7 : 1 }}>
            {busy ? <><span style={{ width: 15, height: 15, borderRadius: "50%", border: "2px solid var(--acc-ink)", borderTopColor: "transparent", animation: "ab-spin .7s linear infinite" }} /> {tl.generating}</> : <><Icon name="sparkle-fill" size={18} /> {tl.generateN.replace("{n}", String(cfg.cost))}</>}
          </button>
        ...
        <div className="ab-canvas-col" style={{ display: "flex", flexDirection: "column", minWidth: 0, alignSelf: "start", position: "sticky", top: 0, height: "calc(100dvh - 64px)" }}>
          <div style={{ height: 52, borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8, padding: "0 18px" }}>
            <span className="ab-chip ab-chip-acc" style={{ padding: "4px 10px" }}>
              {busy ? <><span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--acc)", animation: "ab-blink .8s ease infinite" }} /> {tl.rendering}</> : <><Icon name="check" size={12} stroke={3} /> {result ? tl.ready : tl.idle}</>}
            </span>
            <span className="ab-mono" style={{ color: "var(--t-3)" }}>{cfg.canvasMeta}</span>
            <div style={{ flex: 1 }} />
            {result && <button onClick={() => downloadDataUrl(result, `${cfg.active}.png`)} className="ab-btn ab-btn-ghost ab-btn-sm"><Icon name="download" size={14} /> {tl.save}</button>}
          </div>
          <div style={{ flex: 1, minHeight: "calc(100dvh - 116px)", minWidth: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: 36, position: "relative", backgroundImage: "radial-gradient(var(--border) 1px, transparent 1px)", backgroundSize: "22px 22px", overflow: "hidden" }}>
            {busy ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, color: "var(--t-2)" }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 600 }}>{tl.generating}</div>
              ...
              <div onClick={() => fileRef.current?.click()} style={{ textAlign: "center", color: "var(--t-3)", cursor: "pointer", maxWidth: 320 }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 600, color: "var(--t-1)" }}>{tl.uploadToStart.replace("{label}", cfg.sourceLabel.toLowerCase())}</div>
                <div className="ab-body" style={{ fontSize: 13, marginTop: 6 }}>{tl.pickOptions}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ borderLeft: "1px solid var(--border)", padding: 20 }}>
          <div className="ab-eyebrow" style={{ marginBottom: 12 }}>{tl.variations}</div>
          ...
          <button onClick={generate} disabled={busy || !image} className="ab-btn ab-btn-ghost ab-btn-full ab-btn-sm" style={{ marginTop: 12, opacity: busy || !image ? 0.5 : 1 }}><Icon name="sparkle" size={14} /> {tl.regenerate}</button>
          ...
          <button onClick={() => result && downloadDataUrl(result, `${cfg.active}-hd.png`)} disabled={!result} className="ab-btn ab-btn-primary ab-btn-full" style={{ marginTop: 14, opacity: result ? 1 : 0.5 }}><Icon name="download" size={16} stroke={2} /> {tl.downloadHd}</button>

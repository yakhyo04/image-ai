    ...
    try {
      ...
      for (let i = 0; i < MAX_POLLS; i++) {
        ...
        if (data.status === "failed" || !res.ok) {
          throw new Error(data.error ?? v.genFailed);
        }
        // else pending → keep polling
      }
      throw new Error(v.tooLong);
    } catch (e) {
      if (cancelled.current) return;
      setError(e instanceof Error ? e.message : v.genFailed);
      setPhase("error");
    }
  }
    ...
    <DashFrame active="video" title="Product Video">
      <div className="ab-dash-cols" style={{ display: "grid", gridTemplateColumns: "340px 1fr", minHeight: "100%" }}>
        ...
        <div style={{ borderRight: "1px solid var(--border)", padding: 20 }}>
          <div className="ab-eyebrow" style={{ marginBottom: 12 }}>01 · {v.productImage}</div>
          <div
          ...
          >
            {image ? (
              <>
                ...
                {!working && <span style={{ position: "absolute", bottom: 8, right: 8, padding: "4px 9px", borderRadius: 8, background: "oklch(0 0 0 / 0.55)", color: "oklch(1 0 0 / 0.92)", fontSize: 11, fontWeight: 600 }}>{tl.replace}</span>}
              </>
            ) : (
              <>
          ...
          <div className="ab-eyebrow" style={{ margin: "24px 0 12px" }}>02 · {v.orientation}</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {(["9:16", "16:9"] as const).map((a) => (
              ...
              >
                {a === "9:16" ? v.vertical : v.wide}
              </button>
            ))}
          </div>

          <div className="ab-eyebrow" style={{ margin: "24px 0 12px" }}>03 · {v.animationStyle}</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
            {ANIM_PRESETS.map((p) => {
              ...
              return (
                ...
                >
                  {v.presets[p.key as keyof typeof v.presets]}
                </button>
              );
            })}
          </div>
          ...
          />
          <div className="ab-body" style={{ fontSize: 11.5, marginTop: 6 }}>{v.tip}</div>
          ...
          <button onClick={generate} disabled={working || !image} className="ab-btn ab-btn-primary ab-btn-full ab-btn-lg" style={{ marginTop: 16, opacity: working || !image ? 0.6 : 1 }}>
            {working ? <><span style={{ width: 15, height: 15, borderRadius: "50%", border: "2px solid var(--acc-ink)", borderTopColor: "transparent", animation: "ab-spin .7s linear infinite" }} /> {v.renderingBtn}</> : <><Icon name="sparkle-fill" size={18} /> {v.generateN.replace("{n}", String(VIDEO_COST))}</>}
          </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", minWidth: 0 }}>
          <div style={{ height: 52, borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8, padding: "0 18px" }}>
            <span className="ab-chip ab-chip-acc" style={{ padding: "4px 10px" }}>
              {working ? <><span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--acc)", animation: "ab-blink .8s ease infinite" }} /> {tl.rendering}</> : <><Icon name="sparkle-fill" size={12} /> {phase === "done" ? tl.ready : tl.idle}</>}
            </span>
            {working && <span className="ab-mono" style={{ color: "var(--t-3)" }}>{mins}:{String(secs).padStart(2, "0")} {v.elapsed}</span>}
            <div style={{ flex: 1 }} />
            {phase === "done" && videoUrl && <button onClick={download} disabled={exporting} className="ab-btn ab-btn-ghost ab-btn-sm"><Icon name="download" size={14} /> {exporting ? v.exporting : v.download}</button>}
          </div>
          ...
          <div style={{ flex: 1, minHeight: "calc(100dvh - 116px)", position: "relative", backgroundImage: "radial-gradient(var(--border) 1px, transparent 1px)", backgroundSize: "22px 22px", display: "flex", alignItems: "center", justifyContent: "center", padding: 36, overflow: "hidden" }}>
            {phase === "done" && videoUrl ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, maxWidth: "100%" }}>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", justifyContent: "center" }}>
                  ...
                  <button onClick={download} disabled={exporting} className="ab-btn ab-btn-primary" style={{ opacity: exporting ? 0.7 : 1 }}>
                    {exporting ? <><span style={{ width: 14, height: 14, borderRadius: "50%", border: "2px solid var(--acc-ink)", borderTopColor: "transparent", animation: "ab-spin .7s linear infinite" }} /> {v.exporting}</> : <><Icon name="download" size={16} stroke={2} /> {v.downloadMp4}</>}
                  </button>
                  <Link href="/dashboard/gallery" className="ab-btn ab-btn-ghost"><Icon name="gallery" size={16} /> {v.viewInGallery}</Link>
                  <button onClick={reset} disabled={exporting} className="ab-btn ab-btn-ghost"><Icon name="plus" size={15} /> {v.newVideo}</button>
                </div>
                <div className="ab-body" style={{ fontSize: 12 }}>{v.savedNote}</div>
              </div>
            ) : working ? (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, color: "var(--t-1)", textAlign: "center", maxWidth: 360 }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 600 }}>{v.renderingTitle}</div>
                <div className="ab-body" style={{ fontSize: 13 }}>{v.renderingSub}</div>
              </div>
            ) : (
              <div onClick={() => fileRef.current?.click()} style={{ textAlign: "center", color: "var(--t-3)", cursor: "pointer", maxWidth: 340 }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 600, color: "var(--t-1)" }}>{v.emptyTitle}</div>
                <div className="ab-body" style={{ fontSize: 13, marginTop: 6 }}>{v.emptySub}</div>
              </div>
            )}
          </div>
        </div>

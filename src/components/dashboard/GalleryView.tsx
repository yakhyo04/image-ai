...
const FILTERS = ["All", "Infographics", "Videos", "Interiors", "Mockups", "Backgrounds", "Patterns", "Edits"] as const;
...
function relativeWhen(iso: string, g: Dict["dash"]["gallery"]): string {
  const days = daysSince(iso);
  if (days <= 0) return g.today;
  if (days === 1) return g.yesterday;
  return g.daysAgo.replace("{n}", String(days));
}
...
export default function GalleryView({ items }: { items: GalleryItem[] }) {
  ...
  const toolLabel = (k?: string | null) => t.dash.toolLabels[(k ?? "") as keyof Dict["dash"]["toolLabels"]] ?? g.imageFallback;
  ...
  const stats: [string, string, boolean?][] = [
    [String(items.length), g.statTotal],
    [String(new Set(items.map((i) => i.tool)).size), g.statTools, true],
    [String(items.filter((i) => daysSince(i.createdAt) <= 0).length), g.statToday],
    [String(weekCount), g.statWeek],
  ];

  const badges = (it: GalleryItem) => (
    <>
      <div style={{ position: "absolute", top: 10, left: 10, fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.06em", color: "oklch(1 0 0 / 0.85)", background: "oklch(0 0 0 / 0.4)", backdropFilter: "blur(8px)", padding: "3px 8px", borderRadius: 100 }}>{toolLabel(it.tool).toUpperCase()}</div>
      <div style={{ position: "absolute", bottom: 10, right: 10 }}>
        <span className="ab-mono" style={{ fontSize: 9, color: "oklch(1 0 0 / 0.9)", background: "oklch(0 0 0 / 0.4)", backdropFilter: "blur(8px)", padding: "3px 8px", borderRadius: 100 }}>{relativeWhen(it.createdAt, g)}</span>
      </div>
    </>
  );
    ...
    return (
      <div style={{ ...(mode === "grid" ? { aspectRatio: "3 / 4" } : {}), background: TONES[i % 5], position: "relative" }}>
        {isVideo(it) ? (
          ...
          <img src={thumbSrc(it.url)} alt={toolLabel(it.tool)} loading="lazy" decoding="async" style={mediaStyle} />
        )}
        {isVideo(it) && (
      ...
      <div style={{ padding: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
          <div style={{ display: "flex", gap: 6, background: "var(--bg-1)", border: "1px solid var(--border)", borderRadius: 12, padding: 4, flexWrap: "wrap" }}>
            {FILTERS.map((f) => (
              <span key={f} onClick={() => setFilter(f)} style={{ padding: "8px 14px", borderRadius: 9, fontSize: 13, fontWeight: 500, cursor: "pointer", background: f === filter ? "var(--bg-3)" : "transparent", color: f === filter ? "var(--t-1)" : "var(--t-3)" }}>{g.filters[f]}</span>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ position: "relative" }}>
              <button onClick={() => setSortOpen((o) => !o)} className="ab-btn ab-btn-ghost ab-btn-sm" aria-haspopup="menu" aria-expanded={sortOpen}>
                <Icon name="filter" size={15} /> {sort === "newest" ? g.newest : g.oldest}
              </button>
              {sortOpen && (
                <>
                  ...
                  <div role="menu" style={{ position: "absolute", top: 42, right: 0, zIndex: 41, width: 150, background: "var(--bg-1)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "var(--sh-2)", padding: 6 }}>
                    {(["newest", "oldest"] as const).map((s) => (
                      ...
                      >
                        {s === "newest" ? g.newestFirst : g.oldestFirst}
            ...
            <button
              ...
              title={view === "grid" ? g.toMasonry : g.toGrid}
              aria-label={g.toggleLayout}
        ...
        {shown.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, padding: "80px 20px", textAlign: "center", border: "1.5px dashed var(--border-strong)", borderRadius: 16, color: "var(--t-2)" }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: "var(--t-1)" }}>{items.length === 0 ? g.emptyTitle : g.emptyFilterTitle}</div>
            <div className="ab-body" style={{ fontSize: 13, maxWidth: 320 }}>{items.length === 0 ? g.emptySub : g.emptyFilterSub}</div>
          </div>
        ) : view === "grid" ? (

...
export default function Profile({
  ...
  return (
    <SettingsScaffold active="profile" title="Profile" sub="How your account appears across Artboard." footer={false}>
      <form action={formAction}>
        <Card>
          <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1, minWidth: 160 }}>
              <div className="ab-h4" style={{ fontSize: 18 }}>{fullName || "Your name"}</div>
              <div className="ab-body" style={{ fontSize: 13, marginTop: 2 }}>{storeName ? `${storeName} · ` : ""}{email}</div>
            </div>
            <button type="button" className="ab-btn ab-btn-ghost ab-btn-sm"><Icon name="upload" size={14} /> Change avatar</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 24 }}>
            <EditField label="Full name" name="full_name" defaultValue={fullName} placeholder="Your name" />
            <EditField label="Store name" name="store_name" defaultValue={storeName} placeholder="Your store" />
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, marginTop: 4, marginBottom: 24 }}>
          <button type="submit" disabled={pending} className="ab-btn ab-btn-primary" style={{ opacity: pending ? 0.7 : 1 }}><Icon name="check" size={16} stroke={2.4} /> {pending ? "Saving…" : "Save changes"}</button>
        </div>
      </form>

...
export default function EditorView() {
  ...
  const t = useDict();
  const tl = t.dash.tools;
  const ed = tl.editor;

  async function loadFile(file: File) {
    if (!file.type.startsWith("image/")) {
      setError(tl.chooseImage);
      return;
    }
    try {
      ...
      setError(e instanceof Error ? e.message : tl.couldNotRead);
    }
  }
  ...
  async function apply() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : ed.editFailed);
    } finally {
      setBusy(false);
    }
  }
    ...
    <DashFrame active="editor" title="Photo Editor">
      <div className="ab-dash-cols" style={{ display: "grid", gridTemplateColumns: "316px 1fr 296px", minHeight: "100%" }}>
        ...
        <div style={{ borderRight: "1px solid var(--border)", padding: 20 }}>
          <div className="ab-eyebrow" style={{ marginBottom: 12 }}>01 · {ed.sourceImage}</div>
          <div
          ...
          >
            {image ? (
              <>
                ...
                <span style={{ position: "absolute", bottom: 8, right: 8, padding: "4px 9px", borderRadius: 8, background: "oklch(0 0 0 / 0.55)", color: "oklch(1 0 0 / 0.92)", fontSize: 11, fontWeight: 600 }}>{tl.replace}</span>
              </>
            ) : (
              <>
                ...
                <div style={{ fontSize: 13.5, fontWeight: 600, position: "relative" }}>{ed.dropOrBrowse}</div>
          ...
          <div className="ab-eyebrow" style={{ margin: "24px 0 12px" }}>02 · {ed.selectionBrush}</div>
          <button
          ...
          >
            <Icon name="brush" size={16} /> {brushEnabled ? ed.painting : ed.brushRegion}
          </button>
          {brushEnabled && (
            <div style={{ marginTop: 12 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--t-3)", marginBottom: 6 }}><span>{ed.brushSize}</span><span className="ab-mono">{brushSize}px</span></div>
              ...
              <button onClick={() => { canvasRef.current?.clearMask(); setHasMask(false); }} className="ab-btn ab-btn-ghost ab-btn-sm ab-btn-full" style={{ marginTop: 10 }}>{ed.clearSelection}</button>
              <div className="ab-body" style={{ fontSize: 11.5, marginTop: 8 }}>{hasMask ? ed.regionSelected : ed.brushHint}</div>
            </div>
          )}

          <div className="ab-eyebrow" style={{ margin: "24px 0 12px" }}>03 · {ed.describeEdit}</div>
          <textarea
            value={prompt}
          ...
          <button onClick={apply} disabled={busy || !image || !prompt.trim()} className="ab-btn ab-btn-primary ab-btn-full ab-btn-lg" style={{ marginTop: 16, opacity: busy || !image || !prompt.trim() ? 0.6 : 1 }}>
            {busy ? <><span style={{ width: 15, height: 15, borderRadius: "50%", border: "2px solid var(--acc-ink)", borderTopColor: "transparent", animation: "ab-spin .7s linear infinite" }} /> {ed.applying}</> : <><Icon name="magic" size={18} /> {ed.applyN.replace("{n}", "5")}</>}
          </button>
        ...
        <div className="ab-canvas-col" style={{ display: "flex", flexDirection: "column", minWidth: 0, alignSelf: "start", position: "sticky", top: 0, height: "calc(100dvh - 64px)" }}>
          <div style={{ height: 52, borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8, padding: "0 18px" }}>
            <span className="ab-chip ab-chip-acc" style={{ padding: "4px 10px" }}>
              {busy ? <><span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--acc)", animation: "ab-blink .8s ease infinite" }} /> {ed.editing}</> : <><Icon name="magic" size={12} /> {image ? tl.ready : tl.idle}</>}
            </span>
            <span className="ab-mono" style={{ color: "var(--t-3)" }}>{brushEnabled ? ed.brushMode : ed.wholeImage}{history.length > 1 ? ` · ${history.length - 1} ${ed.editsSuffix}` : ""}</span>
            <div style={{ flex: 1 }} />
            {image && <button onClick={() => downloadDataUrl(image, "edited.png")} className="ab-btn ab-btn-ghost ab-btn-sm"><Icon name="download" size={14} /> {tl.save}</button>}
          </div>
          <div style={{ flex: 1, minHeight: "calc(100dvh - 116px)", minWidth: 0, position: "relative", backgroundImage: "radial-gradient(var(--border) 1px, transparent 1px)", backgroundSize: "22px 22px", display: "flex", alignItems: "center", justifyContent: "center", padding: image ? 0 : 36, overflow: "hidden" }}>
            {image ? (
              ...
              <div onClick={() => fileRef.current?.click()} style={{ textAlign: "center", color: "var(--t-3)", cursor: "pointer", maxWidth: 320 }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 600, color: "var(--t-1)" }}>{ed.uploadPhoto}</div>
                <div className="ab-body" style={{ fontSize: 13, marginTop: 6 }}>{ed.uploadHint}</div>
              </div>
            )}
            {busy && (
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "oklch(0 0 0 / 0.4)", backdropFilter: "blur(2px)" }}>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, color: "var(--t-1)" }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{ed.applyingEdit}</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div style={{ borderLeft: "1px solid var(--border)", padding: 20 }}>
          <div className="ab-eyebrow" style={{ marginBottom: 12 }}>{ed.history}</div>
          {history.length === 0 ? (
            <div className="ab-body" style={{ fontSize: 13 }}>{ed.historyEmpty}</div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {history.map((h, i) => (
                <div key={i} onClick={() => setCurrent(h)} style={{ aspectRatio: "1", borderRadius: 11, overflow: "hidden", border: h === image ? "2px solid var(--acc)" : "1px solid var(--border-mid)", position: "relative", cursor: "pointer" }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={h} alt={i === 0 ? ed.original : `${ed.editShort} ${i}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  <span style={{ position: "absolute", bottom: 5, left: 5, fontFamily: "var(--font-mono)", fontSize: 9, color: "oklch(1 0 0 / 0.85)", background: "oklch(0 0 0 / 0.45)", padding: "1px 5px", borderRadius: 4 }}>{i === 0 ? ed.origShort : `#${i}`}</span>
                </div>
              ))}
            </div>
          )}
          {image && (
            <>
              <button onClick={() => downloadDataUrl(image, "edited-hd.png")} className="ab-btn ab-btn-primary ab-btn-full" style={{ marginTop: 18 }}><Icon name="download" size={16} stroke={2} /> {tl.downloadHd}</button>
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <button onClick={() => { setImage(null); setHistory([]); setPrompt(""); }} className="ab-btn ab-btn-ghost" style={{ flex: 1 }}><Icon name="close" size={15} /> {ed.reset}</button>
              </div>
            </>
          )}
        </div>

    ...
    <AuthLayout variant="signup">
      ...
      <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 26 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <AuthField label="Full name" icon="user" name="full_name" placeholder="Dilnoza R." />
          <AuthField label="Store name" icon="shopping-bag" name="store_name" placeholder="Brio Outwear" />
        </div>
        ...
        <button type="submit" disabled={pending} className="ab-btn ab-btn-primary ab-btn-full ab-btn-lg" style={{ marginTop: 4, opacity: pending ? 0.7 : 1 }}>{pending ? "Creating…" : "Create account"} <Icon name="arrow-right" size={17} stroke={2.2} /></button>
      </form>

...
type NavKey = keyof Dict["dash"]["nav"];
...
function titleFor(p: string, t: Dict): string {
  if (p.startsWith("/dashboard/settings")) return t.dash.titles.settings;
  if (p.startsWith("/dashboard/progress")) return t.dash.titles.generating;
  if (p.startsWith("/dashboard/backgrounds")) return t.dash.titles.backgrounds;
  if (p.startsWith("/dashboard/credits")) return t.dash.titles.credits;
  const id = activeFor(p) as NavKey;
  return t.dash.nav[id] ?? t.dash.nav.dashboard;
}

function Row({ t, label, badge, active, collapsed }: { t: SidebarItem; label: string; badge?: string; active: string; collapsed: boolean }) {
  ...
  return (
    ...
    >
      ...
      {!collapsed && <span style={{ flex: 1, fontSize: 13.5, fontWeight: on ? 600 : 500, letterSpacing: "-0.01em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{label}</span>}
      {!collapsed && badge && <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, fontWeight: 600, color: "var(--acc)", background: "var(--acc-soft)", padding: "2px 7px", borderRadius: 100 }}>{badge}</span>}
    </Link>
  );
}
...
function Sidebar({ active, collapsed, credits }: { active: string; collapsed: boolean; credits: number | null }) {
  const t = useDict();
  const label = (id: string) => t.dash.nav[id as NavKey] ?? id;
  const badgeFor = (item: SidebarItem) => {
    if (item.id === "credits" && credits !== null) return String(credits);
    if (item.badge === "NEW") return t.dash.badgeNew;
    return item.badge;
  };
  return (
    <div className="ab-dash-sidebar" style={{ width: collapsed ? 72 : 248, flexShrink: 0, height: "100%", background: "var(--bg)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", padding: collapsed ? "20px 12px" : "20px 16px", transition: "width .2s ease", overflow: "hidden" }}>
      ...
      <Link href="/dashboard/infographics" className="ab-btn ab-btn-primary ab-btn-full" style={{ flexShrink: 0, padding: collapsed ? "12px 0" : "12px 16px", borderRadius: 12, marginBottom: 18 }}>
        <Icon name="plus" size={17} stroke={2.4} />{!collapsed && <span>{t.dash.newGeneration}</span>}
      </Link>
      ...
      <div className="ab-scroll" style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", margin: collapsed ? "0 -12px" : "0 -16px", padding: collapsed ? "4px 12px" : "4px 16px" }}>
        {!collapsed && <div className="ab-eyebrow" style={{ padding: "0 8px 10px", fontSize: 10 }}>{t.dash.toolsHeader}</div>}
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>{TOOLS.map((item) => <Row key={item.id} t={item} label={label(item.id)} badge={badgeFor(item)} active={active} collapsed={collapsed} />)}</div>
        <div style={{ height: 1, background: "var(--border)", margin: "16px 6px" }} />
        {!collapsed && <div className="ab-eyebrow" style={{ padding: "0 8px 10px", fontSize: 10 }}>{t.dash.library}</div>}
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>{LIB.map((item) => <Row key={item.id} t={item} label={label(item.id)} badge={badgeFor(item)} active={active} collapsed={collapsed} />)}</div>
      </div>
      ...
      {!collapsed ? (
        <div style={{ flexShrink: 0, marginTop: 16, borderRadius: 16, padding: 16, background: "linear-gradient(160deg, var(--acc-soft), transparent)", border: "1px solid var(--acc-line)", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 13, fontWeight: 700 }}><Icon name="crown" size={15} stroke={2} style={{ color: "var(--acc)" }} /> {t.dash.freePlan}</div>
            <div className="ab-body" style={{ fontSize: 11.5, marginTop: 6 }}>{t.dash.creditsLeft.replace("{n}", String(credits ?? "—"))}</div>
            <Link href="/dashboard/credits" className="ab-btn ab-btn-primary ab-btn-full ab-btn-sm" style={{ marginTop: 12 }}>{t.dash.managePlan}</Link>
          </div>
        </div>
      ) : (
...
function Topbar({ title, onToggle, credits, user }: { title: string; onToggle: () => void; credits: number | null; user: DashUser | null }) {
  ...
  return (
    <div className="ab-dash-topbar" style={{ height: 64, flexShrink: 0, borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 16, padding: "0 24px", background: "var(--bg)" }}>
      ...
      <Link href="/dashboard/credits" title={t.dash.creditsBilling} style={{ display: "flex", alignItems: "center", gap: 7, height: 40, padding: "0 14px", borderRadius: 11, background: "var(--acc-soft)", border: "1px solid var(--acc-line)", color: "var(--acc)", fontWeight: 700, fontSize: 14, textDecoration: "none" }}><Icon name="bolt" size={15} /> {credits ?? "—"}</Link>
      <button aria-label={t.dash.notifications} className="ab-dash-bell" style={{ width: 40, height: 40, borderRadius: 11, background: "var(--bg-1)", border: "1px solid var(--border)", color: "var(--t-2)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
          ...
          <>
            ...
            <div style={{ position: "absolute", top: 46, right: 0, width: 220, zIndex: 41, background: "var(--bg-1)", border: "1px solid var(--border)", borderRadius: 14, boxShadow: "var(--sh-2)", padding: 8 }}>
              <div style={{ padding: "8px 10px 10px" }}>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: "var(--t-1)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user?.name || t.dash.account}</div>
              ...
              <Link href="/dashboard/settings" onClick={() => setMenuOpen(false)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 9, textDecoration: "none", color: "var(--t-2)", fontSize: 13.5 }}><Icon name="settings" size={16} /> {t.dash.settingsLabel}</Link>
              <form action={logoutAction}>
                <button type="submit" style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 9, background: "transparent", border: "none", color: "var(--err)", fontSize: 13.5, fontFamily: "var(--font)", cursor: "pointer", textAlign: "left" }}><Icon name="shield" size={16} /> {t.dash.logout}</button>
              </form>
            </div>
          </>
        )}
...
const MOBILE_TABS = [
  ...
  { id: "create", icon: "sparkle-fill", href: "/dashboard/infographics" },
  { id: "gallery", icon: "gallery", href: "/dashboard/gallery" },
        ...
        {MOBILE_TABS.map((tb) => {
          ...
          return (
            <Link key={tb.id} href={tb.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "8px 0 6px", borderRadius: 18, cursor: "pointer", textDecoration: "none", background: on ? "var(--bg-2)" : "transparent" }}>
              ...
              <span style={{ fontSize: 10, fontWeight: on ? 600 : 500, color: on ? "var(--t-1)" : "var(--t-3)" }}>{t.dash.mobile[tb.id]}</span>
            </Link>
          );
        })}
      </div>
  ...
  return (
    <div style={{ display: "flex", height: "100dvh", width: "100%", background: "var(--bg)", color: "var(--t-1)", overflow: "hidden", fontFamily: "var(--font)" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, background: "var(--bg)" }}>
        <Topbar title={titleFor(pathname, t)} onToggle={toggle} credits={credits} user={user} />
